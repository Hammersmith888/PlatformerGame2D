...
public class PlayerControler : MonoBehaviour 
{
    ...
    const float speedMultiplayer = 50f;
    ...
    void FixedUpdate(){
        _rb.velocity = new Vector2(_horizontal * speedX * speedMultiplayer * Time.fixedDeltaTime, _rb.velocity.y);

        if(_isJump){ 
            _rb.AddForce(new Vector2(0f, 1450f));
            _isGround = false ;
            _isJump = false ;
        }
        if (_horizontal > 0f && _isFacingRight == false) {
            Flip();
        }
    ...
    private void OnTriggerEnter2D (Collider2D other) {
        LeverArm leverAmrTemp = other.GetComponent<LeverArm>();

        if(other.CompareTag("Finish")) {
            Debug.Log("Worked");
            _isFinish = true;
        }
        if(leverAmrTemp != null) {
            _isLeverArm = true;
